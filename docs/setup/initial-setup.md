...
!!! warning "重要：APIキーは一度しか表示されません"
    APIキーは作成時に一度だけ表示されます。メモ帳などに保存しておきましょう。紛失した場合は、新しいキーを作り直す必要があります。
...
![メニューバー](placeholder-menu-bar.png)
...
![Base URL入力](placeholder-base-url.png)
...
![APIキー入力](placeholder-api-key.png)

#### (3) モデル名

使いたいAIモデルを指定します。

![モデル名入力](placeholder-model-name.png)
...
![接続テストボタン](placeholder-test-connection.png)
...
年齢制限や暴力的な内容を避けたい場合は、その旨を明記してください。

![システムプロンプト](placeholder-system-prompt.png)
*↑ システムプロンプト入力欄のスクリーンショットをここに挿入してください*
